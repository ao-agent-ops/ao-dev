...
class Runner:
    ...
    # PL is plain text, so no pdf upload etc.
    @retry(wait=wait_random_exponential(min=1, max=60), stop=stop_after_attempt(6))
    def get_gpt_pl_request(self, file_content, q_string):
        system2model = {
            "gpt4_pl": "gpt-4-turbo",
            "gpt-4o_pl": "gpt-4o",
            "gpt3.5_pl": "gpt-3.5-turbo-0125",
        }
        response = (
...
def main():
    ...
    parser.add_argument(
        "--model_dir", type=str, default="YOUR OWN MODEL DIR", help="Downloaded model paths."
    )
    parser.add_argument(
        "--initial_folder", type=int, default=0, help="From which folder to begin evaluation."
    )
