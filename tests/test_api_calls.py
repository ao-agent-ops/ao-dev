...
def run_add_numbers_test(program_file, api_type, create_response_func):
    ...
    for input_text, output_text in zip(groundtruth_inputs, cached_outputs):
        ...
        db.execute(
            ...
            (session_id, "mock_model", input_text, input_hash, node_id, 
             create_response_func(output_text), api_type)
        )
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if (msg.get("type") == "graph_update" and 
...
@pytest.mark.parametrize("program_file,api_type,create_response_func", [
...
if __name__ == "__main__":
    ...
    run_add_numbers_test("anthropic_add_numbers.py", "anthropic_messages", create_anthropic_response)
