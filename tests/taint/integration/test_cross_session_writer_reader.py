    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('openai.OpenAI') as mock_openai:
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "user", "content": "Write a short creative story about robots and humans working together."}
                ]
            )
        ...
        assert stored_session_id == writer_session_id, f"Expected writer session {writer_session_id}, got {stored_session_id}"
        assert writer_node_id in stored_taint, f"Expected writer node {writer_node_id} in taint, got {stored_taint}"
        ...
        with patch('openai.OpenAI') as mock_openai:
            ...
            with open(str(content_file), "r") as f:
                ...
                if prev_session_id and taint_nodes:
                    ...
                    print(f"✅ Retrieved taint from previous session: {prev_session_id}, nodes: {taint_nodes}")
                else:
        ...
        assert stored_session_id == reader_session_id, f"Expected reader session {reader_session_id}, got {stored_session_id}"
        assert writer_node_id in stored_taint, f"Writer node {writer_node_id} should be in extended file taint"
        assert reader_node_id in stored_taint, f"Reader node {reader_node_id} should be in extended file taint"
        ...
        edge = {"id": f"e{writer_node_id}-{reader_node_id}", "source": writer_node_id, "target": reader_node_id}
        ...
        print(f"✅ Writer session now has {len(writer_graph['nodes'])} nodes and {len(writer_graph['edges'])} edges")
        ...
        assert reader_session_id not in mock_session_graphs, "Reader should not create its own session"
