...
class StaticRepoGraph:
    ...
    def get_adjacency_list(self) -> Dict[str, list]:
        ...
        return {
            node: sorted(list(self.graph.successors(node)))
    ...
    def run_pyre(self):
        ...
        config = {
            ...
            "source_directories": [self.repo_root],
            "search_path": [self.repo_root],
            "taint_models_path": [os.path.join(self.repo_root, "taint")],
            "exclude": [".*/build/.*", ".*/venv/.*", "*miniconda*"],
            "strict": True,
        }
