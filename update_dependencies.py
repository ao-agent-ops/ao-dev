...
def get_conda_info():
    ...
    env_name = run_command("conda info --json | python -c \"import sys, json; print(json.load(sys.stdin)['active_prefix_name'])\"")
...
def create_conda_environment_file(env_name, conda_packages):
    ...
    conda_env = {
        'name': env_name,
        'dependencies': sorted(conda_packages) + [
            'pip',
            {'pip': ['-e .']}
        ]
    }
...
def update_pyproject_toml(dependencies, is_dev=False):
    ...
    if is_dev:
        ...
        print(f"  (Filtered out {len(dependencies) - len(filtered_deps)} dependencies already in production)")
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Update conda environment and pyproject.toml dependencies')
    parser.add_argument('--dev', action='store_true', help='Update dev dependencies instead of production')
